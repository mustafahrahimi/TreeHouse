...
class TerminalUI:
    ...
    def show_2d(self):
        ...
        level, count, factor, width = 0, 0, 2, 2 
        while count < len(nodes):
            num_elements = pow(factor, level)
            spacing = " " * (pow(factor, self._tree.height() - level + 1) - 1)
            for _ in range(num_elements):
                print(spacing, end='')
                value = str(nodes[count]) if nodes[count] is not None else '*'
                print(f"{value: >{width}}", end=spacing)
                count += 1
            print()  
            level += 1    
